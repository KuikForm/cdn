"use strict";let kuikformSnackBarWrapper,kuikformSnackBar,kuikformStatusText,isInitialized=!1,snackBarActive=!1;const serverEndpoint="https://kuikform.tanmoy.codes/api/v1/submit/";function initPlugin(){isInitialized||(addElements(),kuikformSnackBarWrapper=document.querySelector(".kuikform-snackbar-wrapper"),kuikformSnackBar=document.querySelector(".kuikform-snackbar"),kuikformStatusText=document.querySelector("#kuikform-status-text"),document.querySelectorAll("[kuikform-id] > input[type=file]").forEach(function(a){a.onchange=function(a){a.target.files[0].size>99e6&&(alert("File size should be less than 99 MB"),this.value="")}}),document.querySelectorAll("[kuikform-id]").forEach(function(a){a.addEventListener("submit",async function(d){if(d.preventDefault(),console.log(a.getAttribute("kuikform-id")),a.checkValidity()){showSnackbarWithPromise(2,"Processing...");let e=await serializeArray(a);var f={id:a.getAttribute("kuikform-id"),data:e};showSnackbarWithPromise(2,"Submitting...");var c=await fetch("https://kuikform.tanmoy.codes/api/v1/submit/",{method:"POST",body:JSON.stringify(f),headers:{"Content-type":"application/json"}});console.log(c);var b=await c.json();b.success?toggleSnackbar(1,b.message):toggleSnackbar(0,b.error)}},!1)}))}function addElements(){document.body.innerHTML+=`
